...
import {
  AdminFriendsQueryDto,
  FriendsQueryDto,
  PlayerLookupQueryDto,
...
export class FriendsService {
  ...
  async listFriends(userId: string, query: FriendsQueryDto) {
    ...
    const [total, rows] = await this.prisma.$transaction([
      ...
      this.prisma.friendship.findMany({
        ...
        take: query.limit,
        include: { friend: { select: this.userSelect() } },
      }),
    ]);
    return {
      friends: rows.map((row) =>
        this.friendResponse(row.friend, row.acceptedAt),
      ),
  ...
  async listRequests(
    ...
    return {
      [direction]: rows.map((row) =>
        this.requestResponse((row as any)[relation], row.createdAt),
      ),
      pagination: this.pagination(query, total),
    };
  }

  requestFriend(requesterId: string, addresseeId: string) {
    return this.createFriendRequestTransaction
      .run({ requesterId, addresseeId })
      .then(() => ({ message: "Friend request sent" }));
  }
  makeFriends(userId: string, friendId: string) {
    return this.createFriendshipTransaction.run({ userId, friendId });
  }
  acceptFriendRequest(actorId: string, requesterId: string) {
    return this.acceptFriendRequestTransaction
      .run({ actorId, requesterId, addresseeId: actorId })
      .then(() => ({ message: "Friend request accepted" }));
  }
  declineFriendRequest(actorId: string, requesterId: string) {
    return this.respondFriendRequestTransaction
      .run({ actorId, requesterId, addresseeId: actorId, response: "DECLINED" })
      .then(() => ({ message: "Friend request declined" }));
  }
  cancelFriendRequest(actorId: string, addresseeId: string) {
    return this.respondFriendRequestTransaction
      .run({ actorId, requesterId: actorId, addresseeId, response: "CANCELED" })
      .then(() => ({ message: "Friend request canceled" }));
  }
  removeFriend(userId: string, friendId: string) {
    return this.removeFriendshipTransaction.run({ userId, friendId });
  }
  blockPlayer(userId: string, blockedId: string) {
    return this.blockPlayerTransaction.run({ blockerId: userId, blockedId });
  }
  unblockPlayer(userId: string, blockedId: string) {
    return this.unblockPlayerTransaction.run({ blockerId: userId, blockedId });
  }

  heartbeat(userId: string) {
    return this.heartbeatPresenceTransaction
      .run(userId)
      .then((presence) => this.presenceResponse(presence));
  }
  ...
  async getPresence(userId: string, viewerId?: string) {
    ...
    if (viewerId && viewerId !== userId) {
      ...
      if (!friendship && !user.profile?.isPublic)
  ...
  async adminList(query: AdminFriendsQueryDto) {
        ...
        ? {
            AND: [
              ...(query.userId
                ...
                : []),
              ...(userWhere
                ? [{ OR: [{ requester: userWhere }, { addressee: userWhere }] }]
                : []),
            ],
          }
        : {}),
        ...
        ? {
            AND: [
              ...(query.userId
                ...
                : []),
              ...(userWhere
                ? [{ OR: [{ user: userWhere }, { friend: userWhere }] }]
                : []),
            ],
          }
        : {};
    const presenceWhere: Prisma.PresenceWhereInput = {
      ...(query.userId ? { userId: query.userId } : {}),
      ...(query.online === undefined
        ? {}
        : {
            lastHeartbeatAt: query.online
              ? { gt: new Date(Date.now() - this.presenceWindowMs()) }
              : { lte: new Date(Date.now() - this.presenceWindowMs()) },
          }),
    ...
    const [
      ...
      this.prisma.friendRequest.count({
        where: relationshipWhere(query.status),
      }),
    ...
    return {
      friendships: friendships.map((row) => ({
        id: row.id,
        user: this.adminUser(row.user),
        friend: this.adminUser(row.friend),
        acceptedAt: row.acceptedAt,
      })),
      requests: requests.map((row) => ({
        id: row.id,
        status: row.status,
        requester: this.adminUser(row.requester),
        addressee: this.adminUser(row.addressee),
        createdAt: row.createdAt,
      })),
      presence: presence.map((row) => ({
        user: this.adminUser(row.user),
        ...this.presenceResponse(row),
      })),
      pagination: {
        page,
        limit,
        friendships: friendshipTotal,
  ...
  async player360Social(userId: string) {
    const [friends, incoming, outgoing, presence] =
      await this.prisma.$transaction([
        this.prisma.friendship.findMany({
          where: { userId },
          orderBy: { acceptedAt: "desc" },
        ...
        this.prisma.friendRequest.findMany({
          ...
          orderBy: { createdAt: "desc" },
          take: 100,
          include: { requester: { select: this.userSelect() } },
        }),
        this.prisma.friendRequest.findMany({
          where: { requesterId: userId, status: FriendRequestStatus.PENDING },
          orderBy: { createdAt: "desc" },
          take: 100,
          include: { addressee: { select: this.userSelect() } },
        }),
    ...
    return {
      ...
      incomingRequests: incoming.map((row) =>
        this.requestResponse(row.requester, row.createdAt),
      ),
      outgoingRequests: outgoing.map((row) =>
        this.requestResponse(row.addressee, row.createdAt),
      ),
      presence: this.presenceResponse(presence),
    };
  }
  ...
  private userSelect() {
    return {
      ...
      presence: { select: { lastHeartbeatAt: true, lastSeenAt: true } },
    };
  }
  private adminUser(user: any) {
    return {
      id: user.id,
  ...
  private friendResponse(user: any, acceptedAt: Date) {
    ...
    const response = {
      ...
      online: this.isOnline(presence?.lastHeartbeatAt),
      lastSeen: presence?.lastSeenAt ?? null,
      last_seen: presence?.lastSeenAt ?? null,
      ulid: user.id,
    };
    return response;
  }
  ...
  private presenceResponse(presence: any) {
    return {
      online: this.isOnline(presence?.lastHeartbeatAt),
  ...
  private isOnline(lastHeartbeatAt?: Date) {
    return Boolean(
      lastHeartbeatAt &&
      lastHeartbeatAt.getTime() > Date.now() - this.presenceWindowMs(),
    );
  }
  private presenceWindowMs() {
    ...
    return (
      (Number.isFinite(configured) && configured >= 30 && configured <= 3600
        ? configured
        : PRESENCE_WINDOW_SECONDS) * 1000
    );
  }
  private pagination(query: FriendsQueryDto, total: number) {
    return {
      page: query.page,
      limit: query.limit,
      total,
