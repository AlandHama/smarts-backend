...
export class UpdateLeaderboardTransaction extends PrismaTransaction<{ id: string; dto: UpdateLeaderboardDto }, any> {
  ...
  protected async execute(input: { id: string; dto: UpdateLeaderboardDto }, transaction: Prisma.TransactionClient) {
    const current = await transaction.leaderboard.findUnique({ where: { id: input.id }, select: { id: true, memberType: true, period: true, _count: { select: { rewards: true } } } })
    if (!current) throw new NotFoundException("Leaderboard not found")
    try {
      const memberType = (input.dto.memberType as LeaderboardMemberType | undefined) ?? current.memberType
      const period = (input.dto.period as LeaderboardPeriod | undefined) ?? current.period
      if (input.dto.rewards?.length && period === LeaderboardPeriod.ALL_TIME) throw new ConflictException("All-time leaderboards cannot renew and distribute placement rewards")
      if (input.dto.rewards !== undefined) await replaceLeaderboardRewards(transaction, input.id, memberType, input.dto.rewards)
      else if (memberType !== LeaderboardMemberType.PLAYER && current._count.rewards > 0) throw new ConflictException("Remove leaderboard rewards before changing this board to a non-player leaderboard")
      return await transaction.leaderboard.update({ where: { id: input.id }, data: { ...(input.dto.key ? { key: input.dto.key.trim().toLowerCase() } : {}), ...(input.dto.name ? { name: input.dto.name.trim() } : {}), ...(input.dto.memberType ? { memberType } : {}), ...(input.dto.period ? { period: input.dto.period as LeaderboardPeriod } : {}), ...(input.dto.direction ? { direction: input.dto.direction as LeaderboardDirection } : {}), ...(input.dto.writePolicy ? { writePolicy: input.dto.writePolicy as LeaderboardWritePolicy } : {}), ...(input.dto.active !== undefined ? { active: input.dto.active } : {}), ...(input.dto.metadata !== undefined ? { metadata: input.dto.metadata as Prisma.InputJsonValue } : {}) } })
    } catch (error) {
