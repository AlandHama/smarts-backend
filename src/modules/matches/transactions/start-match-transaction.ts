...
@Injectable()
export class StartMatchTransaction extends PrismaTransaction<
...
> {
  ...
  protected async execute(
    ...
    const match = await transaction.match.findUnique({
      ...
      include: {
        gameDefinition: { select: { key: true } },
        gameConfig: true,
        participants: true,
        rounds: {
    ...
    if (!currentParticipant)
      throw new NotFoundException("Player is not a participant in this match");
    if (match.status !== "CREATED" && match.status !== "STARTED")
      throw new ConflictException("The match cannot be started");
    ...
    if (match.status === "CREATED") {
      ...
      // safe when both players tap at the same time.
      await transaction.matchParticipant.update({
        where: { id: currentParticipant.id },
        data: { readyAt: currentParticipant.readyAt ?? now },
      });
      ...
      const allHumansReady =
        ...
        humanParticipants.every((participant) => participant.readyAt !== null);
      const currentReady = true;
      const readyCount = participants.filter(
        (participant) =>
          participant.participantType === "BOT" || participant.readyAt !== null,
      ).length;
      const requiredCount = participants.length;
      if (!allHumansReady || readyCount < requiredCount) {
        return {
          matchId: match.id,
          status: "CREATED",
    ...
    if (!existingAssignments.length) {
      ...
      const selectedItems = selectServerContent(
        items,
        match.gameConfig.maxQuestions,
        match.serverNonce,
        match.gameDefinition.key,
      );
      if (!selectedItems.length)
        throw new ConflictException(
          "No active server content is configured for this game",
        );
      if (currentParticipant.participantType !== "BOT")
        for (let position = 0; position < selectedItems.length; position += 1) {
          ...
          const assignment = await transaction.matchContentAssignment.create({
            data: {
              matchId: match.id,
              roundId: round.id,
              participantId: currentParticipant.id,
              ...
              assignmentTokenHash: createHash("sha256")
                ...
                .digest("hex"),
              expiresAt: new Date(
                now.getTime() + match.gameConfig.maxMatchDurationSeconds * 1000,
              ),
            },
            include: {
              contentItem: {
    ...
    if (match.status === "CREATED") {
      ...
      await transaction.matchRound.update({
        where: { id: round.id },
        data: { status: "STARTED", startedAt: now },
      });
    }
