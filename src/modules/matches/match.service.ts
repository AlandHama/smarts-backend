...
export class MatchService {
  constructor(
    ...
    private readonly recordEvent: RecordMatchEventTransaction,
  ...
  create(userId: string, dto: CreateMatchDto) {
    return this.createMatch
      .run({ userId, dto })
      .then((value: any) => this.serializeMatch(value, userId));
  }
  ...
  start(matchId: string, userId: string) {
    return this.startMatch
      .run({ matchId, userId })
      .then((value) => this.serialize(value));
  }
  forfeit(matchId: string, userId: string) {
  ...
  async get(matchId: string, userId: string) {
    ...
    return this.serializeMatch({
      ...
      challengeSeed: this.challengeSeedFromHash(rounds[0]?.challengeSeedHash),
      participants: match.participants.map((participant) =>
        this.publicParticipant(participant, userId, undefined, match.id),
      ),
  ...
  private challengeSeedFromHash(hash?: string) {
    ...
    const seed = Number.parseInt(hash.slice(0, 8), 16) % 2147483647;
    return seed || 1;
  }
  ...
  async getSettlement(matchId: string, userId: string) {
    const match = await this.prisma.match.findFirst({
      where: { id: matchId, participants: { some: { userId } } },
      select: { settlement: true, status: true },
    });
    if (!match) throw new NotFoundException("Match not found");
    return this.serialize(
      match.settlement?.settlementJson ?? {
        status: match.status === "REVIEW" ? "REVIEW" : "PENDING",
        matchId,
      },
    );
  }
  ...
  private serializeMatch<T extends Record<string, any>>(
    value: T,
    userId?: string,
  ): T {
    ...
    const match =
      value.match && typeof value.match === "object"
        ? {
            ...value.match,
            participants: Array.isArray(value.match.participants)
              ? value.match.participants.map((participant: any) =>
                  this.publicParticipant(
                    participant,
                    userId,
    ...
    return this.serialize({
      ...value,
      ...(value.match ? { match } : {}),
      ...(!value.match && Array.isArray(value.participants)
        ? {
            participants: value.participants.map((participant: any) =>
              this.publicParticipant(participant, userId, undefined, matchId),
            ),
          }
        : {}),
    });
