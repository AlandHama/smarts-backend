...
export class WalletController {
  ...
  transferQuote(@Query("amount") amount?: string) { return this.walletService.quoteGldTransfer(amount ?? "0") }

  @Post("transfer")
  @ApiOperation({ summary: "Send GLD to another player" })
  transfer(@CurrentUser() user: UserResponseDto, @Body() dto: GldTransferDto) { return this.walletService.transferGld(user.id, dto) }
}
