...
export class ReverseWalletTransaction extends PrismaTransaction<ReverseWalletInput, any> {
  ...
  protected async execute(input: ReverseWalletInput, transaction: Prisma.TransactionClient) {
    ...
    await writePlayerAudit(transaction, { userId: input.userId, actorType: PlayerAuditActorType.ADMIN, action: "WALLET_REVERSAL", entityType: "WalletTransaction", entityId: reversal.id, summary: `Reversed wallet transaction ${original.id}`, changes: { balance: { old: locked.amount, new: after }, direction: { old: original.direction, new: "REVERSAL" } }, metadata: { originalTransactionId: original.id, sourceId } })
