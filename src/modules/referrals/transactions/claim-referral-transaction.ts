...
export class ClaimReferralTransaction extends PrismaTransaction<{ userId: string; dto: ClaimReferralDto }, any> {
  ...
  protected async execute(input: { userId: string; dto: ClaimReferralDto }, transaction: Prisma.TransactionClient) {
    ...
    const override = await transaction.referralPlayerOverride.findUnique({ where: { userId: referralCode.userId } })
    const maxInvites = override?.maxInvitesPerUser ?? config.maxInvitesPerUser
    const enabled = override?.enabled ?? config.enabled
    if (!enabled) throw new BadRequestException("Referrals are currently unavailable")
    if (referralCode.inviteCount >= maxInvites) throw new ConflictException("This referral code has reached its invite limit")
    ...
    if (lockedCode.inviteCount >= maxInvites) throw new ConflictException("This referral code has reached its invite limit")
