...
import { ClaimReferralDto, UpdateReferralConfigDto } from "./dtos"
...
export class ReferralsService {
  ...
  async getAdminState() {
    const config = await this.prisma.referralConfig.upsert({ where: { singletonKey: "default" }, create: { singletonKey: "default" }, update: {} })
    const [total, active, qualified, capped, rewards, recent] = await this.prisma.$transaction([
  ...
  async applyAdRewardShare(transaction: Prisma.TransactionClient, input: { referredUserId: string; adRewardClaimId: string; grossAmount: bigint; currencyCode: string }) {
    ...
    if (!config?.enabled || !referral || referral.status === ReferralStatus.REVOKED) return { playerAmount: input.grossAmount, referralRewardAmount: 0n, referralId: referral?.id ?? null }

    const referredAdCount = referral.referredAdCount + 1
    const remainingCap = config.maxRewardPerReferral > referral.totalRewardedGld ? config.maxRewardPerReferral - referral.totalRewardedGld : 0n
    let reward = referredAdCount >= config.minQualifyingAds ? input.grossAmount * BigInt(config.rewardBps) / 10_000n : 0n
    if (reward > remainingCap) reward = remainingCap
    const qualified = referredAdCount >= config.minQualifyingAds
