...
export class GldEmissionService {
  async estimateAdReward(transaction: Prisma.TransactionClient, input: AdRewardEstimateInput) {
    ...
    const controls = await transaction.gldAdminControl.findUnique({ where: { singletonKey: "default" }, select: { emissionsPaused: true, adDailyGldCap: true, adMaxValidatedAds: true, adMaxRewardPerClaim: true } })
    ...
    const remainingCap = effectiveConfig.adDailyGldCap > earned ? effectiveConfig.adDailyGldCap - earned : 0n
    ...
    if (amount > effectiveConfig.adMaxRewardPerClaim) amount = effectiveConfig.adMaxRewardPerClaim
    if (amount > remainingCap) amount = remainingCap

    return { amount, remainingDailyAds, remainingDailyGldCap: remainingCap - amount, reason: amount > 0n ? null : curveMultiplier === 0 ? "daily-ad-limit-reached" : "daily-gld-cap-reached" }
  }
  ...
  async issueAdReward(transaction: Prisma.TransactionClient, input: EmissionInput) {
    ...
    const effectiveConfig = applyGldPolicyOverrides(config, controls)
    if (controls.emissionsPaused) return { amount: 0n, validatedAds: 0, remainingDailyAds: effectiveConfig.adMaxValidatedAds, remainingDailyGldCap: effectiveConfig.adDailyGldCap, reason: "emissions-paused" }
    const state = await transaction.gldEconomyState.findFirst({ orderBy: { updatedAt: "desc" }, select: { health: true, dailyEmissionBudget: true } })
    const healthMultiplier = effectiveConfig.adHealthMultiplierBps[state?.health ?? "CRITICAL"] ?? 2_000
    ...
    let amount = input.baseAmount * BigInt(curveMultiplier) * BigInt(healthMultiplier) / 100_000_000n
    if (amount > effectiveConfig.adMaxRewardPerClaim) amount = effectiveConfig.adMaxRewardPerClaim
    const remainingCap = effectiveConfig.adDailyGldCap > lockedPlayer.gldEarned ? effectiveConfig.adDailyGldCap - lockedPlayer.gldEarned : 0n
    if (amount > remainingCap) amount = remainingCap
    ...
    return { amount, validatedAds: adNumber, remainingDailyAds: Math.max(effectiveConfig.adMaxValidatedAds - adNumber, 0), remainingDailyGldCap: remainingCap - amount, reason: amount > 0n ? null : curveMultiplier === 0 ? "daily-ad-limit-reached" : "daily-gld-cap-reached" }
  }
