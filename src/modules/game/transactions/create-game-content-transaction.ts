...
import { isMathServerContent } from "../../matches/utilities/server-content"
...
export class CreateGameContentTransaction extends PrismaTransaction<CreateGameContentDto, any> {
  ...
  protected async execute(input: CreateGameContentDto, transaction: Prisma.TransactionClient) {
    ...
    if (input.gameKey.trim().toLowerCase() === "math" && !isMathServerContent({ prompt: input.prompt, options })) throw new BadRequestException("Math content must be a numeric expression with four numeric answer options")
