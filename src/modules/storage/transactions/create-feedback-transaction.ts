...
import { FeedbackEntity, Prisma } from "@prisma/client"
...
export class CreateFeedbackTransaction extends PrismaTransaction<CreateFeedbackInput, any> {
  ...
  protected async execute(input: CreateFeedbackInput, transaction: Prisma.TransactionClient) {
    ...
    return transaction.playerFeedback.create({ data: { userId: input.userId, categoryId: category.id, entity: input.dto.entity, entityId: input.dto.entityId, description: input.dto.description.trim(), metadata: input.dto.metadata as Prisma.InputJsonValue | undefined } })
  }
}
