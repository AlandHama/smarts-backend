...
export class GiftsService {
  ...
  async stats(viewerUserId: string, targetUserId: string) {
    await this.assertCanView(viewerUserId, targetUserId)
    const rows = await this.prisma.playerGift.findMany({ where: { OR: [{ senderUserId: targetUserId }, { recipientUserId: targetUserId }] }, select: { senderUserId: true, gldPrice: true, burnedAmount: true } })
    return this.serialize({ sent: rows.filter((row) => row.senderUserId === targetUserId).length, received: rows.filter((row) => row.senderUserId !== targetUserId).length, totalGldSent: rows.filter((row) => row.senderUserId === targetUserId).reduce((sum, row) => sum + row.gldPrice, 0n), totalGldBurned: rows.filter((row) => row.senderUserId === targetUserId).reduce((sum, row) => sum + row.burnedAmount, 0n) })
  }
