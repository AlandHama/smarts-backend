...
export function OverviewView() {
  ...
  const generatedLabel = data?.period.to
    ...
    <Stack spacing={3.5}>
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
            ...
            {data && (
              <Chip
                ...
                variant="outlined"
                color="success"
                label={`Updated ${generatedLabel}`}
              />
            )}
          </Stack>
          <Typography variant="h4" fontWeight={850} sx={{ mt: 1.3 }}>
            SMARTS analytics & reports
          </Typography>
          <Typography color="text.secondary" sx={{ mt: 0.7, maxWidth: 900 }}>
            Understand acquisition, engagement, retention, gameplay quality,
            progression, economy, and platform health from the Railway source of
            truth.
          </Typography>
        </Box>
        <Select
      ...
      <Grid container spacing={1.5}>
        {[
          ...
          [
            "Active players",
            kpis?.periodActiveUsers,
            "Unique players in range",
          ...
          [
            "Matches settled",
            kpis?.matchesSettled,
            `${percent(kpis?.completionRate ?? 0)} completion`,
          ...
          [
            "Answer accuracy",
            kpis?.accuracy,
            `${fmt(kpis?.acceptedAnswers ?? 0)} accepted answers`,
          ...
          [
            "Purchase value",
            kpis?.purchaseValue,
            `${fmt(kpis?.completedPurchases ?? 0)} completed purchases`,
          ...
          [
            "Hours played",
            kpis?.totalPlayHours,
            "Estimated active player time",
            <AccessTimeRoundedIcon />,
            "#45d5a2",
          ],
          [
            "Daily play average",
            kpis?.averageDailyPlayHours,
          ...
          [
            "Average session",
            kpis?.averageSessionMinutes,
            "Minutes per player session",
            <AccessTimeRoundedIcon />,
            "#f4c95d",
          ],
            ...
            <MetricCard
              ...
              value={
                loading && !data
                  ? null
                  : typeof value === "number"
                    ? formatMetric(String(label), value)
                    : "0"
              }
      ...
      <Grid container spacing={2}>
        <Grid size={{ xs: 12, lg: 8 }}>
          <ReportCard
            title="Play-time trend"
            subtitle="Estimated active player hours and verified match time by UTC day."
          >
            <TrendChart
              points={trends}
        ...
        <Grid size={{ xs: 12, lg: 4 }}>
          <PlayTimeReport data={data} loading={loading} />
        </Grid>
      </Grid>
      ...
      <Grid container spacing={2}>
        <Grid size={{ xs: 12, lg: 8 }}>
          <ReportCard
            ...
            subtitle="Daily active users, new registrations, and completed matches."
          >
            <TrendChart points={trends} series={engagementSeries} />
          </ReportCard>
        </Grid>
        <Grid size={{ xs: 12, lg: 4 }}>
          <RetentionCard data={data} loading={loading} />
        </Grid>
      </Grid>
      ...
      <Grid container spacing={2}>
        <Grid size={{ xs: 12, lg: 7 }}>
          <ReportCard
            title="Gameplay quality"
            subtitle="Accepted server events and verified answer correctness."
          >
            <TrendChart points={trends} series={gameplaySeries} />
          </ReportCard>
        </Grid>
        <Grid size={{ xs: 12, lg: 5 }}>
          <GameBreakdown games={data?.games ?? []} loading={loading} />
        </Grid>
      </Grid>

      <Grid container spacing={2}>
        <Grid size={{ xs: 12, lg: 6 }}>
          <ProgressionReport data={data} loading={loading} />
        </Grid>
        <Grid size={{ xs: 12, lg: 6 }}>
          <EconomyReport data={data} loading={loading} />
        </Grid>
      </Grid>

      <AdMobReport data={data} loading={loading} />

      <Grid container spacing={2}>
        <Grid size={{ xs: 12, lg: 5 }}>
          <GameplayReport data={data} loading={loading} />
        </Grid>
        <Grid size={{ xs: 12, lg: 7 }}>
          <AudienceReport data={data} loading={loading} />
        </Grid>
      </Grid>

      <Grid container spacing={2}>
        <Grid size={{ xs: 12 }}>
          <HealthReport data={data} loading={loading} />
        </Grid>
      </Grid>
    </Stack>
  );
...
function MetricCard({
  ...
  return (
    <Card sx={{ p: 2.2, height: "100%" }}>
      ...
      {value === null ? (
        <Skeleton width={100} height={48} sx={{ mt: 1.3 }} />
      ) : (
        <Typography variant="h4" sx={{ mt: 1.3, fontWeight: 850 }}>
          {value}
        </Typography>
      )}
...
function TrendChart({
  ...
  const values = series.flatMap((item) =>
    points.map((point) => Number(point[item.key as keyof typeof point]) || 0),
  );
...
function PlayTimeReport({
  ...
  const rows = data
    ? ([
        ...
        [
          ...
          `${data.playTime.averageDailyHours.toFixed(2)}h`,
        ],
        ...
        [
          "Longest session",
          formatDuration(data.playTime.longestSessionMinutes * 60),
        ],
  ...
  return (
    <ReportCard
      title="Time spent"
      subtitle="Session activity is capped at 12 hours per session; match time is shown separately."
    >
      <Stack spacing={1.15}>
        {loading && !data ? (
...
function RetentionCard({
  ...
  const rows = data
    ? ([
        ["D1", data.retention.day1],
        ["D7", data.retention.day7],
        ["D30", data.retention.day30],
      ] as const)
    : [];
  return (
    <ReportCard
      title="Player retention"
      subtitle="Cohort return rate from server activity."
    >
      <Stack spacing={2.2}>
        {loading && !data ? (
          ...
          rows.map(([label, item]) => (
            <Box key={label}>
              <Stack
                ...
                sx={{ mb: 0.6 }}
              >
                <Typography fontWeight={750}>{label} retention</Typography>
                <Typography fontWeight={800} color="primary.light">
                  {percent(item.rate)}
                </Typography>
              </Stack>
              <Box
              ...
              <Typography variant="caption" color="text.secondary">
                {fmt(item.retained)} returned of {fmt(item.eligible)} eligible
                players
              </Typography>
            </Box>
          ))
        ) : (
          <EmptyState text="Retention needs at least one mature cohort." />
        )}
      </Stack>
    </ReportCard>
  );
...
function GameBreakdown({
  ...
  return (
    <ReportCard
      ...
      subtitle="Which game modes attract play and produce quality answers."
    >
      <Stack spacing={1.7}>
          ...
          games.map((game) => (
            <Box key={game.key}>
              <Stack direction="row" justifyContent="space-between" spacing={2}>
                <Typography fontWeight={750}>{game.name}</Typography>
                <Typography variant="body2" color="text.secondary">
                  {fmt(game.matches)} matches · {percent(game.accuracy)}{" "}
                  accuracy
                </Typography>
              </Stack>
              <Box
              ...
              >
                <Box
                  sx={{
                    width: `${(game.matches / max) * 100}%`,
                    height: "100%",
                    borderRadius: 5,
                    background: "linear-gradient(90deg, #8b7dff, #52c7f5)",
                  }}
                />
              </Box>
              <Stack direction="row" spacing={1.5} sx={{ mt: 0.4 }}>
                <Typography variant="caption" color="text.secondary">
                  {fmt(game.settled)} settled
                </Typography>
                <Typography variant="caption" color="text.secondary">
                  {fmt(game.review)} review
                </Typography>
                <Typography variant="caption" color="text.secondary">
                  Avg score {fmt(game.averageScore)}
                </Typography>
              </Stack>
            </Box>
          ))
...
function ProgressionReport({
  ...
  return (
    <ReportCard
      ...
      subtitle="Current progression reach and XP movement during the selected period."
    >
      <Stack spacing={1.6}>
        {loading && !data ? (
          ...
          data.progression.map((item) => (
            ...
            >
              ...
              <Box sx={{ minWidth: 0, flex: 1 }}>
                ...
                <Typography variant="caption" color="text.secondary">
                  {fmt(item.players)} players · average{" "}
                  {fmt(item.averagePoints)} points · top level{" "}
                  {fmt(item.highestStep)}
                </Typography>
              </Box>
              <Chip
                size="small"
                color="success"
                variant="outlined"
                label={`+${fmt(item.periodDelta)} XP`}
              />
            </Stack>
          ))
        ) : (
          <EmptyState text="No progression records yet." />
        )}
      </Stack>
    </ReportCard>
  );
...
function EconomyReport({
  ...
  const rows = data
    ? ([
        ["Credits issued", fmt(data.kpis.walletCredits), "success"],
        ["Currency spent", fmt(data.kpis.walletDebits), "warning"],
        ["Ad claims", fmt(data.economy.adClaims), "info"],
        ["Granted ad claims", fmt(data.kpis.grantedAdClaims), "info"],
        [
          "Paid reward requests",
          fmt(data.economy.paidRewardRequests),
          "primary",
        ],
        [
          "Fulfilled paid rewards",
          fmt(data.kpis.fulfilledPaidRewards),
          "primary",
        ],
      ] as const)
    : [];
  return (
    <ReportCard
      ...
      subtitle="Ledger-backed currency flow, ads, purchases, and paid reward fulfillment."
    >
      <Stack spacing={1.2}>
        {loading && !data ? (
          <Skeleton variant="rectangular" height={180} />
          ...
          rows.map(([label, value, color]) => (
            ...
            >
              <Stack direction="row" spacing={1} alignItems="center">
                <MonetizationOnRoundedIcon
                  fontSize="small"
                  color={color as "success" | "warning" | "info" | "primary"}
                />
        ...
        {data && (
          <Stack
            ...
            sx={{ pt: 0.5 }}
          >
            <Typography color="text.secondary">Rejected ad claims</Typography>
            <Typography color="error.main" fontWeight={800}>
              {fmt(data.economy.rejectedAdClaims)}
            </Typography>
          </Stack>
        )}
        {!data && !loading && <EmptyState text="No economy records yet." />}
      </Stack>
    </ReportCard>
  );
}
...
function AdMobReport({
  ...
  const report = data?.admob;
  const rows = report
    ? ([
        ["Impressions", fmt(report.kpis.impressions)],
        ["Clicks", fmt(report.kpis.clicks)],
        ["Match rate", percent(report.kpis.matchRate)],
        ["Show rate", percent(report.kpis.showRate)],
        [
          "Estimated earnings",
          `${report.connection?.currencyCode || "USD"} ${report.kpis.estimatedEarnings.toFixed(4)}`,
        ],
      ] as const)
    : [];
  return (
    <ReportCard
      ...
      subtitle="Synchronized AdMob network performance and estimated earnings for the selected period."
    >
      <Stack spacing={1.2}>
        {loading && !data ? (
          <Skeleton variant="rectangular" height={180} />
          ...
          <>
            ...
            >
              ...
              <Typography variant="body2" color="text.secondary">
                Connected to {report.connection?.publisherId} · last sync{" "}
                {report.connection?.lastSyncAt
                  ? new Date(report.connection.lastSyncAt).toLocaleString()
                  : "pending"}
              </Typography>
            </Stack>
            {rows.map(([label, value]) => (
          ...
          <EmptyState text="Connect AdMob to include impressions, clicks, and earnings in the main analytics report." />
        )}
      </Stack>
    </ReportCard>
  );
  ...
  const rows = data
    ? ([
        [
          ...
          formatDuration(data.gameplay.averageMatchDurationSeconds),
        ],
        ...
        [
          "Average answer time",
          formatDuration(data.kpis.averageAnswerTimeMs / 1000),
        ],
      ] as const)
    : [];
  return (
    <ReportCard
      ...
      subtitle="Match pacing, bot coverage, and server review signals."
    >
      <Stack spacing={1.15}>
        {loading && !data ? (
...
function AudienceReport({
  ...
  return (
    <ReportCard
      title="Audience mix"
      subtitle="Active players by country and authenticated client type."
    >
      <Stack spacing={2}>
        {loading && !data ? (
          ...
          <>
            <Box>
              ...
              <Stack spacing={0.8} sx={{ mt: 1 }}>
                {(data?.countries ?? []).slice(0, 5).map((item) => (
                  <Stack
                    key={item.countryCode}
                    direction="row"
                    justifyContent="space-between"
                  >
                    <Typography fontWeight={700}>{item.countryCode}</Typography>
                    <Typography color="text.secondary">
                      {fmt(item.activeUsers)} active · {fmt(item.newPlayers)}{" "}
                      new
                    </Typography>
                  </Stack>
                ))}
              ...
              >
                {(data?.devices ?? []).map((item) => (
                  <Chip
                    ...
                    size="small"
                    variant="outlined"
                    label={`${item.type} · ${fmt(item.users)} players`}
                  />
                ))}
              </Stack>
            </Box>
...
function HealthReport({
  ...
  const values = data
    ? ([
        ["Online now", data.health.onlinePlayers],
        ["Searching tickets", data.health.searchingTickets],
        ["Active matches", data.health.activeMatches],
        ["Failed outbox", data.health.failedOutbox],
        ["Open feedback", data.health.openFeedback],
      ] as const)
    : [];
  return (
    <ReportCard
      title="Live game health"
      subtitle="Operational signals that explain whether players can play successfully."
    >
        ...
        {loading && !data ? (
          ...
          values.map(([label, value]) => (
            <Card
              key={label}
              variant="outlined"
              sx={{ p: 1.6, flex: "1 1 150px", bgcolor: "rgba(7,12,26,.22)" }}
            >
              <Stack direction="row" spacing={1} alignItems="center">
                <AccessTimeRoundedIcon
                  fontSize="small"
                  color={
                    label === "Failed outbox" && value > 0 ? "error" : "success"
                  }
                />
      ...
      <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 2 }}>
        <CheckCircleRoundedIcon color="success" fontSize="small" />
        <Typography variant="caption" color="text.secondary">
          Metrics are calculated from sessions, accepted events, settlements,
          ledgers, and durable admin records.
        </Typography>
      </Stack>
    </ReportCard>
  );
