...
export function Player360View({ userId, onBack, onOpenMatch360 }: { userId: string; onBack: () => void; onOpenMatch360?: (matchId: string) => void }) {
  ...
  return <Stack spacing={3}>
    ...
    <Section icon={<CalendarTodayRoundedIcon />} title="Match history" description="Recent matches involving this player, including result, score, and the complete authoritative timeline."><Box sx={{ overflowX: 'auto' }}><Table><TableHead><TableRow><TableCell>Game</TableCell><TableCell>Mode</TableCell><TableCell>Result</TableCell><TableCell>Score</TableCell><TableCell>Status</TableCell><TableCell>Played</TableCell><TableCell align="right">Open</TableCell></TableRow></TableHead><TableBody>{data.matches.map((match) => <TableRow key={match.id} hover><TableCell><Typography fontWeight={700}>{match.match.gameDefinition.name}</Typography><Typography variant="caption" color="text.secondary">{match.match.gameDefinition.key}</Typography></TableCell><TableCell>{match.match.mode}</TableCell><TableCell><Chip size="small" label={match.result} color={match.result === 'WIN' ? 'success' : match.result === 'LOSS' || match.result === 'FORFEIT' ? 'error' : 'default'} /></TableCell><TableCell>{match.finalScore ?? '—'}</TableCell><TableCell>{match.match.status}</TableCell><TableCell>{shortDate(match.createdAt)}</TableCell><TableCell align="right">{onOpenMatch360 && <Button size="small" variant="outlined" onClick={() => onOpenMatch360(match.match.id)}>Match 360</Button>}</TableCell></TableRow>)}{!data.matches.length && <TableRow><TableCell colSpan={7} align="center" sx={{ py: 4 }}><Typography color="text.secondary">No match history yet.</Typography></TableCell></TableRow>}</TableBody></Table></Box></Section>
