...
import {
  ...
  ProgressionsView,
} from "../components/ProgressionsView";
import {
  LeaderboardView,
  SeasonManagerPanel,
...
const viewPaths: Record<
  Exclude<AdminView, "player360" | "match360">,
  string
> = {
  overview: "/",
...
function routeState() {
  if (typeof window === "undefined")
    return { view: "overview" as AdminView, id: null as string | null };
  const relative = window.location.pathname
    .replace(/^\/system-admin\/?/, "")
    .split("/")
    .filter(Boolean);
  if (relative[0] === "players" && relative[1])
    return { view: "player360" as AdminView, id: relative[1] };
  if (relative[0] === "matches" && relative[1])
    return { view: "match360" as AdminView, id: relative[1] };
  const view = (Object.entries(viewPaths).find(
    ([, path]) => path.replaceAll("/", "") === relative.join(""),
  )?.[0] || "overview") as AdminView;
  return { view, id: null };
}
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    api<any>("/overview")
      .then(() => setAdmin({ username: "Administrator" }))
      .catch(() => clearSession())
  ...
  const go = (nextView: AdminView, id?: string | null, replace = false) => {
    const path =
      nextView === "player360" && id
        ? `/system-admin/players/${id}/`
        : nextView === "match360" && id
          ? `/system-admin/matches/${id}/`
          : `/system-admin${viewPaths[nextView as Exclude<AdminView, "player360" | "match360">] || "/"}`;
  ...
  return (
    <ThemeProvider theme={adminTheme}>
      ...
      {checking ? null : !admin ? (
        ...
        <AdminShell
          view={view}
          onViewChange={(nextView) => go(nextView)}
          onLogout={signOut}
        ...
        >
          ...
          {view === "player360" && player360Id && (
            <Player360View
              ...
              onOpenMatch360={openMatch360}
            />
          )}
          {view === "matches" && <MatchesView onOpenMatch360={openMatch360} />}
          {view === "match360" && match360Id && (
            <Match360View matchId={match360Id} onBack={backToMatches} />
          )}
